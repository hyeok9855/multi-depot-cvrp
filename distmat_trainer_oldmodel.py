...
class MDCVRPTrainer:
    def __init__(
        ...
        ### Model ###
        self.actor = Actor(env=self.env, **model_params["actor_params"]).to(self.device)  # type: ignore
        self.critic = Critic(env=self.env, **model_params["critic_params"]).to(self.device)  # type: ignore
