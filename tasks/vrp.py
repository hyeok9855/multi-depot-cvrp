...
class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, input_size, max_load, max_demand, seed=None, custom_locations=None):
        ...
        if custom_locations is not None:  # 만약 사용자가 지정한 위치 정보가 있다면,
            locations = custom_locations  # 해당 정보를 Tensor로 변환하여 사용합니다.

            # if locations.shape[2] != input_size + 1:
            #     raise ValueError('Custom locations do not match specified input size')

        else:  # 그렇지 않으면 기존대로 랜덤한 위치 정보를 생성합니다.
        ...
        demands = torch.randint(4, max_demand + 1, dynamic_shape)
...
def distance(static, tour_indices):
