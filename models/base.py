...
class Attention(nn.Module):
    ...
    def forward(self, att_query: torch.Tensor, att_key: torch.Tensor) -> torch.Tensor:
        """
        Args:
            att_query: (batch_size, 1, query_hidden_size)
            att_key: (batch_size, key_hidden_size, num_keys)
        """
        batch_size = att_query.shape[0]

        att_query = att_query.transpose(1, 2).expand((-1, -1, att_key.shape[2]))
        att_hidden = torch.cat((att_key, att_query), 1)

        # Broadcast some dimensions so we can do batch-matrix-multiply
        v = self.v.expand(batch_size, 1, self.hidden_size)
        W = self.W.expand(batch_size, self.hidden_size, -1)

        attns = torch.bmm(v, torch.tanh(torch.bmm(W, att_hidden)))  # (batch_size, 1, num_keys)
        attns = F.softmax(attns, dim=-1)
        return attns  # (batch_size, 1, num_keys)
