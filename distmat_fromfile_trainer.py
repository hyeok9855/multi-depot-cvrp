...
class MDCVRPTrainer:
    def __init__(
        ...
        self.model_params["critic_params"]["dist_encoder_params"].update(
            {"input_size": self.env.n_custs + self.env.n_agents}
        )
        ...
        # Paths
        self.result_dir = Path(self.trainer_params["result_dir"]) / input_dir_name / exp_name
        ...
        if self.use_tensorboard:
            self.tb_log_dir = Path(self.trainer_params["tb_log_dir"]) / input_dir_name / exp_name
    ...
    def validate_epoch(self, epoch: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if epoch == 1:
            self.valid_dataset = self.env.generate_data(self.trainer_params["valid_n_samples"], val=True)
...
if __name__ == "__main__":
    env_params = {
        "input_dir": "data/N20_M3_D3",  # The directory contains `nodes.csv` and `distmat_train` directory
        "phase": "train",
        "one_by_one": False,
        "no_restart": True,
    ...
    model_params = {
        "actor_params": {
            ...
            "dist_encoder_params": {"hidden_size": 64, "input_size": 10},  # input_size is same as svd_q
