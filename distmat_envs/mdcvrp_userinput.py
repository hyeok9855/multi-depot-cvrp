...
class MDCVRPEnv:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.n_custs = None  # to be set later
        assert dimension == 2, "Only 2D is supported for now"
        self.dimension = dimension
        self.distmats = None  # to be set later
        self.device = device
    ...
    def create_distmat_from_userinput(self):
        """
        Prompt the user input for the environment
        User input should be 2d list of matrix
        """
        user_input = eval(input("Enter a 2D array (e.g., [[1, 2], [3, 4]]): "))
        squared_matrix = np.array(user_input) ** 2
        self.n_custs = squared_matrix.shape[0] - self.n_agents
        self.distmats = squared_matrix[np.newaxis]  # (1, n_agents + n_custs, n_agents + n_custs)

    def reset(self, td: TensorDict | None = None, batch_size: int = 1) -> TensorDict:
        """Reset the Environment"""
        assert batch_size == 1, "Batch size should be 1 for now"
        if td is not None:
            assert td.shape[0] == 1, "Batch size should be 1 for now"
            self.batch_size = 1
            return td
    ...
    def step(self, td: TensorDict) -> TensorDict:
        ...
        assert self.n_custs is not None, "n_custs is not set"
    ...
    def generate_data(self, batch_size: int) -> TensorDict:
        assert batch_size == 1, "Batch size should be 1 for now"
        self.create_distmat_from_userinput()  # n_custs and distmats are set here
        assert self.n_custs is not None, "n_custs is not set"
        assert self.distmats is not None, "distmats is not set"
        ...
        # distance matrix
        dist_mat = (
            torch.from_numpy(self.distmats).float().to(self.device)
        )  # (1, n_agents + n_custs, n_agents + n_custs)
