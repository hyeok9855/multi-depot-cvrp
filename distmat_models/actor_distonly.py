...
class Actor(nn.Module):
    ...
    Args:
        ...
        node_encoder_params: Node encoder params
        agent_encoder_params: Agent encoder params
        dist_encoder_params: RNN input encoder params
    ...
    def __init__(
        ...
        node_encoder_params: dict[str, Any],
        ...
        super().__init__()
        self.env = env
        self.node_encoder = Encoder(**node_encoder_params)
        ...
        self.arrive_map = nn.Sequential(
            nn.Linear(
                node_encoder_params["hidden_size"] + dist_encoder_params["hidden_size"],
        ...
        self.depart_map = nn.Sequential(
            nn.Linear(
                agent_encoder_params["hidden_size"] + dist_encoder_params["hidden_size"],
                ptrnet_params["hidden_size"],
            ),
            nn.ReLU(),
            nn.Linear(ptrnet_params["hidden_size"], ptrnet_params["hidden_size"]),
    ...
    def n_nodes(self) -> int:
        ...
        return self.env.n_agents + self.env.n_custs
    ...
    def get_node_x(self, obs_td: TensorDict) -> torch.Tensor:
        ...
        batch_size = len(obs_td)
        device = obs_td.device

        depot_or_cust = torch.cat(
            [
                torch.zeros((batch_size, self.env.n_agents), device=device),
                torch.ones((batch_size, self.env.n_custs), device=device),
            ],
            dim=1,
        )
        # (batch_size, n_nodes) ... n_nodes = n_custs + n_agents
        ...
        aug_demand = torch.cat(
            [-torch.ones((batch_size, self.env.n_agents), device=device), demand], dim=1
        )  # (batch_size, n_nodes)
        node_x = torch.cat([depot_or_cust.unsqueeze(-1), aug_demand.unsqueeze(-1)], dim=-1)  # (batch_size, n_nodes, 2)
        node_x = node_x.transpose(1, 2)  # (batch_size, 2, n_nodes)

        assert node_x.shape[1] == self.node_encoder.input_size
        return node_x

    def get_agent_x(self, obs_td: TensorDict) -> torch.Tensor:
        capacity = cast(torch.FloatTensor, obs_td["remaining_capacity"].unsqueeze(-1)).clone()
        # (batch_size, n_agents, 1)
        agent_x = capacity.transpose(1, 2)  # (batch_size, 1, n_agents)
    ...
    def get_action(
        ...
        # Node embeddings
        node_x = self.get_node_x(obs_td)  # (batch_size, 2, n_nodes)
        node_z = self.node_encoder(node_x)  # (batch_size, hidden_size, n_nodes) // embeddings for nodes to arrive at

        # Agent embeddings
        agent_x = self.get_agent_x(obs_td)  # (batch_size, 1, n_agents)
