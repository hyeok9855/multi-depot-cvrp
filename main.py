...
from models import DRL4TSP, StateCritic
...
def train(
    ...
    **kwargs
    ...
    save_dir = os.path.join("results", task, str(num_nodes), NOW)
    ...
    for epoch in range(5):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            actor_loss.backward()
            torch.nn.utils.clip_grad_norm_(actor.parameters(), max_grad_norm)  # type: ignore
            actor_optim.step()

            critic_optim.zero_grad()
            critic_loss.backward()
            torch.nn.utils.clip_grad_norm_(critic.parameters(), max_grad_norm)  # type: ignore
            critic_optim.step()
        ...
        valid_dir = os.path.join(save_dir, "images", "%s" % epoch)
        ...
        if mean_valid < best_reward:
            ...
            save_path = os.path.join(checkpoint_dir, "best_actor.pt")
            torch.save(actor.state_dict(), save_path)

            save_path = os.path.join(checkpoint_dir, "best_critic.pt")
            torch.save(critic.state_dict(), save_path)
...
def train_vrp(args):
    ...
    test_dir = os.path.join("results", args.task, str(args.num_nodes), NOW, "test")
...
if __name__ == "__main__":
    ...
    if args.task == "vrp":
        ...
        raise NotImplementedError("Only VRP is implemented at this time")
