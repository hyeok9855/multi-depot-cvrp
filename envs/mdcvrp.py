...
class MDCVRPEnv:
    ...
    def step(self, td: TensorDict) -> TensorDict:
        ...
        agent_node_idx = cast(torch.LongTensor, td["agent_node_idx"])  # (batch_size, n_agents)
        ...
        loc_idx = torch.where(depot_bool, selected_agent, self.n_agents + selected_node)  # (batch_size, 1)
        loc_src = gather_by_index(loc, loc_idx, dim=1, squeeze=False)  # (batch_size, 1, 2)
        ...
        agent_node_idx = torch.scatter(input=agent_node_idx, index=selected_agent, dim=1, src=selected_node)
        ...
        used_capacity = gather_by_index(demand[at_node], selected_node[at_node], dim=1, squeeze=False)
        ...
        demand[at_node] = torch.scatter(input=demand[at_node], index=selected_node[at_node], dim=1, value=0.0)
        ...
        action_mask = torch.full_like(action_mask, True)  # (batch_size, n_agents, n_nodes + 1)
        # mask out the node (or depot) that the agent is currently at
        action_mask = torch.scatter(input=action_mask, index=(agent_node_idx + 1).unsqueeze(-1), dim=2, value=False)
        # if demand is 0, mask out the node
        action_mask[:, :, 1:] = torch.where(demand.unsqueeze(1) == 0, False, action_mask[:, :, 1:])
        # if the demand of the node is larger than the remaining capacity, mask out the node
        action_mask[:, :, 1:] = torch.where(
            demand.unsqueeze(1) > remaining_capacity.unsqueeze(2), False, action_mask[:, :, 1:]
        )
        ...
        # if no action is available for any agent, unmask the first depot
        ...
        # reward is the negative of the cumulative length of all the agents
    ...
    def generate_data(self, batch_size: int) -> TensorDict:
        ...
        # index of the agents (-1 for depot)
        agent_node_idx = torch.full((batch_size, self.n_agents), -1, dtype=torch.int64, device=self.device)
        ...
        action_mask = torch.full(
            (batch_size, self.n_agents, self.n_nodes + 1), True, dtype=torch.bool, device=self.device
        )
        ...
        return TensorDict(
            {
                ...
                "agent_node_idx": agent_node_idx,  # (batch_size, n_agents)
                "remaining_capacity": remaining_capacity,  # (batch_size, n_agents)
                "cum_length": cum_length,  # (batch_size, n_agents)
                "action_mask": action_mask,  # (batch_size, n_agents, n_nodes + 1)
            },
    ...
    def load_data_from_csv(self, path: dict[str, Path | str], n_agents: int, vehicle_capacity: int) -> TensorDict:
        ...
        agent_loc = loc[:, : self.n_agents, :].clone()
        agent_node_idx = torch.full((1, self.n_agents), -1, dtype=torch.int64, device=self.device)
        ...
        action_mask = torch.full((1, self.n_agents, self.n_nodes + 1), True, dtype=torch.bool, device=self.device)
        action_mask[:, :, 0] = False  # Agents cannot visit the depot at the beginning
        ...
        return TensorDict(
            {
                ...
                "agent_node_idx": agent_node_idx,  # (1, n_agents)
                "remaining_capacity": remaining_capacity,  # (1, n_agents)
                "cum_length": cum_length,  # (1, n_agents)
                "action_mask": action_mask,  # (1, n_agents, n_nodes + 1)
            },
            batch_size=1,
        )
    ...
    def rand_action(self, td: TensorDict) -> TensorDict:
        ...
        flat_action = torch.multinomial(action_mask.flatten(start_dim=-2).float(), num_samples=1).squeeze(-1)
        action = torch.stack([flat_action // n_actions, flat_action % n_actions], dim=-1)
        action[..., 1] -= 1  # -1 for the depot
    ...
    def render(self, td: TensorDict, actions=None, ax=None, save_path: Path | str | None = None):
        ...
        for act in actions:
            agent_idx, node_idx = act
            agent_idx, node_idx = int(agent_idx), int(node_idx)
            if node_idx == -1:
                node_idx = agent_idx
            else:
                node_idx += self.n_agents
            ...
            agent_loc_idx_before[agent_idx] = node_idx
...
if __name__ == "__main__":
    batch_size = 3
    env = MDCVRPEnv(n_nodes=20, n_agents=5, device="cpu")
